...
**Mumble side-channel.** `instance.setMumble(cb)` registers an opt-in callback that receives a short opaque tail snippet (~120 chars) of the model's in-progress output and the milliseconds since the previous emit, while a turn is in flight. Throttled to ≤ 1 emit / second; fires only while the instance is `busy`; resets on every terminator, interrupt, and `destroy()`. Pass `undefined` to disable. Without a registered callback, no buffer is built and no timer is armed — the side-channel costs nothing when not used. Useful for "thinking…" indicators in CLIs and TUIs that would otherwise sit silent under slow models. The sample is for human eyes only — Claude `text` and `thinking` blocks contribute, `tool_use` does not; Codex contributes its `item/agentMessage/delta` text. Consumers must not parse the sample.
...
`ConversationTurn` surfaces final messages only. Intermediate tool-use events, reasoning traces, and streaming deltas are not exposed as structured events; the optional `setMumble(cb)` side-channel surfaces them as opaque ≤120-char text samples for human-visible progress only — consumers must not parse the sample.
