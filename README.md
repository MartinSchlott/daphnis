...
**Lifecycle events.** `instanceEvents` is a typed `EventEmitter<InstanceEventMap>` exposing four events: `instance:added`, `instance:removed`, `instance:ready`, `instance:meta-changed`. Subscribe once and react to lifecycle without polling `listInstances()` / `getSessionId()`. Listeners must not throw — Node's `EventEmitter` propagates synchronous throws back to the emit site, which sits inside the wrapper constructor / handshake / `setMeta` path.
...
`interrupt()` has no internal timeout and three race outcomes. The promise resolves only after both the provider's cancel ack *and* the in-flight turn's terminator have arrived — so on resolution `busy` is already clear and the next `sendMessage` works without "Already processing". A hung CLI cannot be healed from inside the wrapper; race the returned promise against your own `AbortSignal` / timer and call `destroy()` if you want a hard stop. Three terminator races: (a) real interrupt — no assistant turn appended, no `onError`, `interrupt()` resolves; (b) natural completion (turn finished before the cancel landed) — the assistant turn IS appended and `onConversation` / `onMessage` fire as in the normal path, because `getTranscript()` is in-memory only and silently dropping a successfully produced answer would erase it permanently; `interrupt()` still resolves; (c) real provider failure during the cancel race — `onError` fires AND `interrupt()` rejects with the same error. Pending interrupt promises are also rejected on `proc.on('exit')`, `proc.on('error')`, `stdin.on('error')`, and `destroy()` — a dead child does not leak a forever-pending promise. Whether the interrupted turn lands in the on-disk JSONL is undocumented for both providers; daphnis does not edit the on-disk JSONL, the in-memory transcript is authoritative for the live instance.
Lifecycle events fire synchronously, but on different code paths. `instance:added` and `instance:removed` fire inside `register` / `unregister`; `instance:added` fires before `createAIConversation()` returns. For `instance:removed`, the entry is deleted from the map *before* the event is emitted — a subscriber that calls `listInstances()` from inside the handler sees the instance already gone, but the event payload carries the final `InstanceInfo` snapshot (id, sessionId, pid, meta) captured before the delete. `instance:ready` fires from a `registry.emitReady(id)` helper invoked by each wrapper immediately after its internal `ready` flag flips to `true`: for Claude that happens inside the constructor, *after* `instance:added`, so observers see `added → ready` on the same tick `createAIConversation()` returns (and `info.sessionId` is still `null` because `system/init` has not arrived yet); for Codex it happens later, after the `thread/start` / `thread/resume` handshake resolves, so `info.sessionId` already carries the captured `threadId`. The flag is set exactly once per instance lifetime; the event therefore fires at most once per id. `instance:meta-changed` fires synchronously inside `setMetaFor` whenever the meta slot is updated for a known id, with payload `[info, prev]`: `info.meta` carries the new value, `prev` carries whatever was stored before. Suppressed for unknown ids (silent no-op preserved). Not emitted on initial `register` — the initial meta value is carried by the `instance:added` payload. No equality check between `prev` and the new value: every call emits, even if the caller hands in the exact same reference twice. Events are forward-only: late subscribers do not receive replayed history. Listeners must not throw — Node's `EventEmitter` propagates synchronous throws back to the emit site, which sits inside the wrapper constructor (Claude `ready`), the async handshake flow (Codex `ready`), or `setMeta` (`meta-changed`). Spawn-failure semantics — the wrapper registers before any async failure can fire, so an ENOENT or Codex handshake failure produces `instance:added` followed shortly by `instance:removed` (no `instance:ready` if the handshake never completes). The standard `EventEmitter` listener-leak warning fires past 10 listeners; raise the cap with `instanceEvents.setMaxListeners(n)` if you actually need that many.
